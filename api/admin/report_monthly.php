<?php
    ...
    $sql = "SELECT DATE_FORMAT(t.tgl_selesai, '%M', 'id_ID') as 'bulan',  COUNT(td.id_transaksi) as 'jumlah_item', SUM(td.harga) as 'total_pendapatan'
            FROM transaksi as t
            INNER JOIN transaksi_detail as td
            ON t.id_transaksi = td.id_transaksi
            WHERE t.status = 'Pesanan Selesai'
            GROUP BY MONTHNAME(t.tgl_selesai)";
    $stmt = $conn->query($sql);
    $stmt->execute();
